...
import { useGlobalLeaderboard } from "@/lib/hooks/UseOracleGame";
...
export function AccountPanel() {
  ...
  // Derive the connected user's XP from the global leaderboard
  const { data: leaderboard } = useGlobalLeaderboard();
  const myXp = useMemo(() => {
    if (!address || !leaderboard) return 0;
      ...
      <Dialog open={isModalOpen} onOpenChange={setIsModalOpen}>
        ...
        <DialogContent className="brand-card border-2">
          <DialogHeader>
            ...
            <DialogDescription>
              Connect your MetaMask wallet to join games and earn XP
            </DialogDescription>
          </DialogHeader>
      ...
      <div className="flex items-center gap-4">
        ...
        <div className="brand-card px-4 py-2 flex items-center gap-3">
          ...
          <div className="flex items-center gap-1">
            <span className="text-sm font-semibold text-accent">{myXp}</span>
            <span className="text-xs text-muted-foreground">XP</span>
          </div>
        </div>

        {/* Trigger for the detail modal */}
        <DialogTrigger asChild>
      ...
      <DialogContent className="brand-card border-2">
        ...
        <div className="space-y-4 mt-4">
          ...
          <div className="brand-card p-4 space-y-2">
            ...
            <p className="text-2xl font-bold text-accent">{myXp}</p>
          </div>
